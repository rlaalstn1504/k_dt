...
if __name__ == "__main__":
    ...
    # # 영상 선택
    # selected_video_index = st.selectbox("영상을 선택하세요.", range(len(example_videos)), format_func=lambda x: f"영상 {x+1}")
    # selected_video = example_videos[selected_video_index]

    # # 선택된 영상에 따라 질문 목록을 동적으로 제공할 수 있습니다.
    # # 여기서는 예시를 단순화하기 위해 모든 영상에 대해 동일한 질문 목록을 사용합니다.
    # selected_question_index = st.selectbox("질문을 선택하세요.", range(len(example_questions_ko)), format_func=lambda x: example_questions_ko[x])

    # # 실행 버튼
    # if st.button("실행"):
    #     # 선택된 영상과 질문에 대한 처리 로직
    #     video_example = example_videos[selected_video_index]
    #     question = example_questions_ko[selected_question_index]
    #     answer = ask_gpt3(f"이 질문에 니가 마치 의사인 것 처럼 가정하고 답변해줘: {question}")
    #     col1.write(f'질문 : {question}')
    #     col1.write(f'답변 : {answer}')
    #     col1.video(video_example)
